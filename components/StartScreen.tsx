...
const StartScreen: React.FC<StartScreenProps> = ({ onStart, lang, onLanguageChange }) => {
  ...
  return (
    <div className="absolute inset-0 flex flex-col items-center justify-center z-50 text-white font-sans p-6 bg-black/30 backdrop-blur-sm transition-all duration-1000">
      <div className="max-w-md w-full bg-slate-900/90 p-8 rounded-2xl border border-slate-700 shadow-2xl backdrop-blur-xl relative overflow-hidden animate-fade-in">
        ...
        {/* Language Selector */}
        <div className="absolute top-4 right-4 flex gap-1 bg-slate-800/80 p-1 rounded-lg border border-slate-700/50 z-20">
          <button
            onClick={() => onLanguageChange('en')}
            className={`px-2.5 py-0.5 text-xs font-bold rounded-md transition-all ${lang === 'en' ? 'bg-cyan-600 text-white' : 'text-slate-400 hover:text-white'}`}
          >
            EN
          </button>
          <button
            onClick={() => onLanguageChange('bm')}
            className={`px-2.5 py-0.5 text-xs font-bold rounded-md transition-all ${lang === 'bm' ? 'bg-cyan-600 text-white' : 'text-slate-400 hover:text-white'}`}
          >
            BM
          </button>
        </div>
