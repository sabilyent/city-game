...
import { Language, TRANSLATIONS, getBuildingName } from './translations';
...
function App() {
  ...
  const fetchNewGoal = useCallback(async () => {
    ...
    const newGoal = await generateCityGoal(gridRef.current.length ? statsRef.current : statsRef.current, gridRef.current, langRef.current);
    if (newGoal) {
  ...
  const fetchNews = useCallback(async () => {
    ...
    const news = await generateNewsEvent(statsRef.current, null, langRef.current);
  ...
  useEffect(() => {
    ...
    addNewsItem({ 
      ...
      text: TRANSLATIONS[langRef.current].terrainComplete, 
      type: 'positive' 
    });
  ...
  const handleTileClick = useCallback((x: number, y: number) => {
    ...
    if (tool === BuildingType.None) {
      if (currentTile.buildingType !== BuildingType.None) {
        ...
        if (currentStats.money >= demolishCost) {
            ...
            addNewsItem({
              id: Date.now().toString(), 
              text: TRANSLATIONS[langRef.current].demolitionCostError, 
              type: 'negative'
            });
        }
      }
    ...
    if (currentTile.buildingType === BuildingType.None) {
      if (currentStats.money >= buildingConfig.cost) {
        ...
        const localizedName = getBuildingName(tool, langRef.current);
        addNewsItem({
          id: Date.now().toString() + Math.random(), 
          text: TRANSLATIONS[langRef.current].insufficientFunds.replace('{name}', localizedName), 
          type: 'negative'
        });
      }
    }
  ...
  const handleClaimReward = () => {
    if (currentGoal && currentGoal.completed) {
      ...
      addNewsItem({
        id: Date.now().toString(), 
        text: TRANSLATIONS[langRef.current].goalAchieved.replace('{reward}', currentGoal.reward.toString()), 
        type: 'positive'
      });
      setCurrentGoal(null);
      fetchNewGoal();
  ...
  return (
    <div className="relative w-screen h-screen overflow-hidden selection:bg-transparent selection:text-transparent bg-sky-900">
      {/* 3D Rendering Layer - Always visible, providing background for start screen */}
      <IsoMap 
        grid={grid} 
